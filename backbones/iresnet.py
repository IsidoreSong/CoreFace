...
class IResNet(nn.Module):
    ...
    def double_dropout(self, x):
        mask1 = (torch.rand_like(x) > self.dropout.p).float()
        mask2 = (torch.rand_like(x) > self.dropout.p).float()
        x2 = x.clone()
        x = x * mask1 / (1 - self.dropout.p)
        x2 = x2 * mask2 / (1 - self.dropout.p)
        return x, x2
