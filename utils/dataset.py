...
class MXFaceDataset2(Dataset):
    def __init__(self, root_dir, local_rank, origin_idx, length=None):
        super(MXFaceDataset2, self).__init__()
        self.transform = transforms.Compose(
            [
                transforms.ToPILImage(),
                transforms.RandomHorizontalFlip(),
        ...
        if header.flag > 0:
            ...
            self.imgidx = np.array(list(self.imgrec.keys))
        self.imgidx = self.imgidx[origin_idx.cpu().numpy()]
        if length is not None:
            self.imgidx = self.imgidx[:length]
