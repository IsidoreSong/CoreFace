...
class Torch_model_inter:
    def __init__(self, model_file, image_size=(112, 112)):
        ...
        self.device = 0
        try:
            self.model = SoftmaxNet(loss="ArcFace").to(self.device)
    ...
    def __call__(self, imgs):
        ...
        imgs = (imgs - 127.5) * 0.0078125
        with torch.no_grad():
            output = self.model.backbone(self.torch.from_numpy(imgs).to(self.device).float())
        # self.device = self.devices[(self.device + 1) % len(self.devices)]
        return output.cpu().detach().numpy()
...
def face_align_landmark(img_pth, landmark, image_size=(112, 112), method="similar"):
...
def get_aligned(img_names, landmarks, batch_size):

    executor = ThreadPoolExecutor(max_workers=8)
    executor = ProcessPoolExecutor(max_workers=10)
    ndimage_lis = []
    for i, res in tqdm(enumerate(executor.map(face_align_landmark, img_names, landmarks)), desc="Aligning", total=len(img_names)):
        np.save(os.path.join(args.data_path, args.subset, "img_np",
                             os.path.basename(img_names[i])), np.array(res))
...
def get_embeddings(model_interf, img_names, landmarks, batch_size=64, flip=True):
    ...
    for batch_id in tqdm(range(0, len(img_names), batch_size), "Embedding", total=steps):
        ...
        for res in executor.map(np.load,
             [os.path.join(args.data_path, args.subset, "img_np",
                          os.path.basename(img)+'.npy')
                for img in img_names[batch_id : batch_id + batch_size]
             ]
            ):
            ndimages.append(res)
    ...
    def __init__(self, model_file, data_path, subset, batch_size=64, force_reload=False, restore_embs=None):
        ...
        if model_file != None:
            if model_file.endswith(".h5"):
                ...
                interf_func = Torch_model_inter(model_file)
...
def plot_roc_and_calculate_tpr(scores, names=None, label=None):
    ...
    logging.info(tpr_result_df.to_markdown())
...
def parse_arguments(argv):
    ...
    parser.add_argument("-m", "--model_file", type=str, default='/dataset/model/WaveFace_m2_25/250000model.pth', help="Saved model, keras h5 / pytorch jit pth / onnx / mxnet")
    parser.add_argument("-d", "--data_path", type=str, default="/dataset/IJB/IJB_release", help="Dataset path containing IJBB and IJBC sub folder")
    ...
    parser.add_argument("-D", "--model_dir", type=str, help="Save label data, useful for plot only")
    ...
    parser.add_argument("-A", "--save_aligned", action="store_true", help="Save aligned image by batch")
    parser.add_argument("-l", "--load_aligned", action="store_true", help="Load aligned image by batch")
...
def test_model_dir(args):
    # default = '/dataset/model/WaveFace_alter_k2'
    for item in tqdm(os.listdir(args.model_dir)):
        if int(item[-15:-9]) > 230000:
            print(item)
            if int(item[-15:-9]) in [261556, 284300]:
                continue
            args.save_result = args.save_result.replace('250000', item[-15:-9])
            tt = IJB_test(os.path.join(args.model_dir, item), args.data_path, args.subset,
...
if __name__ == "__main__":
    ...
    if args.save_aligned:
        ...
        get_aligned(img_names, landmarks, args.batch_size)
        exit(0)
